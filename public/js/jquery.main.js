...
//引用相关页面模版
  var xhr_dlogin = $.get( "public/views/dialog-login.html", function( dialog_login ) {
    $( "body" ).append( dialog_login );
  });

  var xhr_toolbar = $.get( "public/views/toolbar.html", function( toolbar ) {
    $( "body" ).append( toolbar );
  });
  
  var xhr_mchat = $.get( "public/views/main-chat.html", function( main_chat ) {
    $( "body" ).append( main_chat );
  });

  var xhr_options = $.get( "public/views/options.html", function( options ) {
    $( "body" ).append( options );
  });
//页面加载完成后done
  $.when( xhr_dlogin, xhr_toolbar, xhr_mchat, xhr_options ).done(function(r1, r2, r3, r4) {

    main_set_conf();      //从config.js中获取所有设定
    main_set_i18n();      //从i18n_xx.js获取所有设定
    main_set_theme( conf_theme_default );   //改变主题
    ...
    //Main chat title bar  主聊天标题栏
    main_chat_status( i18n.disconnected, "offline" );
    
    //0 chat users at begining   初始时0用户
    ...
    //Init state of bar, collapse or expand   初始化BAR状态，展开可折叠
    ...
    //Click for expand and collapse   点击展开或折叠
    ...
    $( document ).on("click", ".button-user", function() {
      ...
      //Append the Dialogid
      main_append_dialog( userid );    //添加对话框
      main_set_dialog( userid );   //新用户对话框

      //Append the user to chat
      main_chat_user_new( userid, status, name );  //主窗口增加新用户
    });

    //Custom message      自定义信息
    ...
    //Mouse over-out effect of user   鼠标over-out效果
    ...
    //Click in user in chat box    聊天box中点击点击用户
    ...
    //Button close search for clean it  搜索关闭按钮（清除信息）
    ...
    //Bar text button open chat   点击bar按钮打开chat
    ...
    $( document ).on("click", ".user-status", function() {
      ...
      if ( id == "user-status-online" && now != "public/images/button_online.png" ) {
        status = "online";
        label  = i18n.connected;
      }

      else if ( id == "user-status-busy" && now != "public/images/busy.png" ) {
        status = "busy";
        label  = i18n.busy;
      }

      else if ( id == "user-status-offline" && now != "public/images/button_offline.png" ) {
        status = "offline";
        label  = i18n.offline;
      }
    ...
    //Minus icon in main chat    主窗口最小化按钮
    $( document ).on("click", "#min-main-chat", function() {
      $( "#main-users-resizer" ).hide();
    });

    //User status msg    用户状态消息，自定义消息  
    ...
    //If enter key is pressed on custom message   如果在自定义消息上按下回车键
    ...
    //如果在cloudflow中自动填写用户名邮箱
    function autofill(){
      // alert("目前為測試版本，歡迎大家使用！\n\n如有問題請撥打12652或郵件溝通！\n\n祝您工作愉快！\n\n湯其華 2016.10.14");
      ...
      if (current_user.username != "guest"){
        ...
        if (my_email.value==""){
         my_email.value ="請輸入您的郵箱，且不可與他人重複！";
        }
    ...
    //Box Dialog for new user   新用户对话框
    function main_set_dialog ( id, user ) {
      // alert("userinfo::::"+id+";"+user);
      ...
      $( "#Dialog" + id ).dialog({
        ...
        open: function(event, ui) {
          // alert("open:::"+id+"  ;"  +user);
          //Set language in Dialog and init   初始化对话框语言
          ...
          if ( $( this ).data( "init" ) != 1 ) {
            ...
            //Change text for the default lang   根据默认语言改变文本
            ...
            //Set hide 'is writing...'  隐藏（is writing）
            main.parent().find( "#iswriting" ).first().addClass( "no-display" );

            //Change flag at init option  初始化  改变标志
            main.data( "init", 1 );

            //Textarea click    文本框点击
            main.parent().find( "textarea" ).click(function() {
              main_chat_user_alert( id, 1 );
            });

            //Textarea focus fancy in and out  文本框聚焦样式（进入或出去）
            ...
            var typingTimeout;
            //Textarea send form when key enter is pressed  当文本框按回车键时发送
            main.find( "textarea" ).first().keyup(function( e ) {
              ...
              //Progressbar of char in textarea   文本框中的进度条
            ...
            // 发送图片
            main.find( "#sendImage").first().on('change', function() {
                if (this.files.length != 0) {
                  ...
                  reader.onload = function(e) {
                    ...
                    if (msg !== "") {
                  main.parent().find( "#progressbar-char" ).progressbar( "option", "value", 0 );
                  socket.emit('img', { 'user': user, 'msg': msg }, function (data) {
                    var recv = JSON.parse(data);
                    append_img_me(msg, main, recv.date);
                    // FIXME
                    // Set dialog position
                    main.dialog( "option", "position", { my: "right bottom", at: "right top-3", of: "#user-button-"+id, collision: "flip, none" });
                  });
            ...
            main.find( "#sendFile").first().on('change', function() {
                if (this.files.length != 0) {
                  ...
                  reader.onload = function(e) {
                    ...
                    var recv = data;
                    //var recv = JSON.parse(data);
                    append_file_me(msg, main, recv.date, filename, filetype);
                    // FIXME
                    // Set dialog position
                    main.dialog( "option", "position", { my: "right bottom", at: "right top-3", of: "#user-button-"+id, collision: "flip, none" });
                  });
    ...
    //Socket emit send data   socket发送用户状态
    ...
    function append_img_me ( msg, main, date ) {
      var box = main.parent().find(".box-body");
      var me = box.find('.direct-chat-messages').last();
      var scroll = main.parent().find( ".direct-chat-messages" );

      if (me.children().last().attr('id') == 'me') {
        me.children().find('.direct-chat-text').last().append("\
          <a href='" + msg + "' target='_blank'><img width=160 src='" + msg + "'/></a>")
      } else {
        me.append("\
          <div class='direct-chat-msg right' id='me'>\
            <div class='direct-chat-info clearfix'>\
    ...
    function append_img_he ( msg, main, name, date, avatar ) {
      var fullname = '';
      var fname = name.split(' ');
      var box = main.parent().find(".box-body");
      var he = box.find('.direct-chat-messages').last();
      ...
      if (he.children().last().attr('id') == 'he') {
        he.children().find('.direct-chat-text').last().append("\
          <a href='" + msg + "' target='_blank'><img width=160 src='" + msg + "'/></a>")
      } else {
        he.append("\
          <div class='direct-chat-msg' id='he'>\
    ...
    //Function for Open chat box  打开chat box的函数
    ...
    //Debug errors with alert  弹出调试错误
    if ( conf_debug == true ) {
    ...
    //Set position on resize of the window    设置窗口的大小设置位置
    ...
    function main_chat_status ( text , status ) {
      ...
      if ( status == "online" )
        $( "#rerun-img" ).attr( "src", "public/images/button_online.png" );
      else if ( status == "busy" )
        $( "#rerun-img" ).attr( "src", "public/images/busy.png" );
      else if ( status == "offline" )
        $( "#rerun-img" ).attr( "src", "public/images/button_offline.png" );
    ...
    // Delete the user in main chat, but in message box,   在主聊天中删除用户，但保留消息框，
    // you must be able to read the conversation  你必须能够读到过去的对话
    ...
    //Set the num of available users  要用用户数量
    ...
    function main_set_theme( theme ) {
      var theme_css = "public/themes/" + theme + "/jquery-ui.min.css";    
      $( "#theme" ).attr( "href" , theme_css );
      return false;
    }

    //Change position on resize window   改变窗口的位置
...
//         'reconnection': true,'reconnectionDelayMax':30000,'reconnectionDelay':1000}
      //这一段与原来写法不一样，原来没有connect,端口号也不是和网站放一块的。
      socket = io( 'http://'+ conf_server+':'+conf_port , {
        'connect timeout': 5000
      });
    }
    //监听模块
    function socket_handle() {
      ...
      socket.on('chat', function (recv) {
        //由于文件传送过来的recv类型为是object,其它为json，所以要在这里做判断。
        //有空，把服务端的都改成obj
